...
forbes_url = 'https://www.forbes.com/advisor/money-transfer/currency-converter/usd-try/?'
...
xe_params     = urllib.parse.urlencode({'from': 'USD',
                                        'to': 'TRY',
                                        'amount': '1'})
...
def drawbox(string, charset):
    ...
    width = 0
    for i in range(0, len(a)):
        if unilen(a[i]) > width: width = unilen(a[i])
    for i in range(0, len(a)):
...
def sb_wgb():
    ...
    try:
       ...
       try:
           cds = wgbsoup.find_all('div', {"class": "w3-cell font-open-sans"}
                                 )[0].get_text().split()[0]
           perc = wgbsoup.find_all('p', string=re.compile('CDS value changed')
                                 )[0].get_text().split()
